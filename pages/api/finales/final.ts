...
import { FinalAp, DatosFinal, User, PartidoAp } from "../../../models";
...
const updateFinal = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    if (resultado === "local") {
      await partidoTercer[0].updateOne({ campeon: partido.local });
      await partidoTercer[0].updateOne({ sub: partido.visitante });
    }

    if (resultado === "visitante") {
      await partidoTercer[0].updateOne({ campeon: partido.visitante });
      await partidoTercer[0].updateOne({ sub: partido.local });
    }
